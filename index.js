...
async function safePrompt(questions) {
  try {
    ...
    if (error.isTtyError || error.message.includes('force closed')) {
      console.log(
        chalk.red(
          '\n\n🦜 You must not be a bird nerd. Thanks for playing. Goodbye!'
        )
      );
      process.exit(0);
    } else {
...
console.log(
  chalk.magenta(
    figlet.textSync('Birds of a Feather Quiz Game!', {
      font: 'standard',
...
console.log(chalk.cyan('Welcome to the Birds of a Feather Quiz Game!'));
...
async function askQuestion() {
  ...
  if (shouldContinue) {
    ...
    console.log(
      chalk.bold.green(
        `\n 🦜 Not a bird nerd! Thanks for playing. Your score is: ${score}/${currentQuestionIndex}`
      )
    );
  }
}
